...
import connectdb, { closeDatabase, onDatabaseConnected } from './config/mongodb.js';
...
onDatabaseConnected(startDatabaseServices);

// Database connection
connectdb({ retry: true }).catch(err => {
  logger.error('Database connection error', { error: err.message, stack: err.stack });
  logger.warn('Server will keep running and retry MongoDB in the background. Database-dependent features may fail until MongoDB is available.');
});
