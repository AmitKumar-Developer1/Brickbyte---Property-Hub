...
# BrickByte — Backend API ⚙️

_The core REST API server driving the BrickByte real estate platform._
...
# MONGO_URI=mongodb+srv://<user>:<pass>@<cluster>.mongodb.net/brickbyte?retryWrites=true&w=majority
...
<details>
<summary><strong>4. Local MongoDB Options</strong></summary>

Use one of these options before starting the backend.

```bash
# Option A: Docker from the backend folder
docker compose up -d mongo

# Option B: MongoDB installed on your machine
...
The readiness response should show `checks.database.status` as `healthy`.
