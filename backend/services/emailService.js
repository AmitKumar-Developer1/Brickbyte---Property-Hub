...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@brickbyte.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to BrickByte - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your BrickByte Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your BrickByte Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to BrickByte Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@brickbyte.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `BrickByte Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'BrickByte Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'BrickByte Account Reactivated - Welcome Back!';
