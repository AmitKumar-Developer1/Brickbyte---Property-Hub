...
// Signed-in users can publish listings directly; no admin review gate is applied.
 ...
 */
function getLocalUploadUrl(req, file) {
    const protocol = req.protocol || 'http';
    const host = req.get('host') || `localhost:${process.env.PORT || 4000}`;
    return `${protocol}://${host}/uploads/${file.filename}`;
}

async function uploadImages(files, req) {
    return Promise.all(
        files.map(async (file) => {
            try {
                ...
                logger.warn("ImageKit upload failed; using local upload URL", {
                    error: error.message,
                    file: file.filename,
                });
...
/** POST /api/user/properties — create a new listing and publish it immediately */
export const createUserListing = async (req, res) => {
    try {
        ...
        const imageUrls = await uploadImages(files, req);
        ...
        res.status(201).json({ success: true, message: 'Listing published successfully', property });
    } catch (error) {
...
/** PUT /api/user/properties/:id — edit an owned listing and keep it live */
export const updateUserListing = async (req, res) => {
    try {
        ...
        const files = req.files || [];
        if (files.length > 0) {
            imageUrls = await uploadImages(files, req);
        }
        ...
        res.json({ success: true, message: 'Listing updated and published successfully', property: updated });
    } catch (error) {
