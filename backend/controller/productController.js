...
const uploadPropertyImages = async (files, req) => {
    return Promise.all(
        files.map(async (item) => {
            try {
                ...
                console.warn("ImageKit upload failed; using local upload URL:", error.message);
                return getLocalUploadUrl(req, item);
            }
        })
    );
};
...
const addproperty = async (req, res) => {
    try {
        ...
        const imageUrls = await uploadPropertyImages(images, req);
