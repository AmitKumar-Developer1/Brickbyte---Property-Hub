    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          ...
          to: recipientEmail,
          subject: `Viewing Appointment ${status.charAt(0).toUpperCase() + status.slice(1)} - BrickByte`,
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Viewing Scheduled - BrickByte',
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    // User cancellation requires an authenticated owner. Guest bookings should
    // be cancelled by an admin route, not this public user endpoint.
    if (!req.user || !appointment.userId || appointment.userId._id.toString() !== req.user._id.toString()) {
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Appointment Cancelled - BrickByte',
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Meeting Link Updated - BrickByte',
