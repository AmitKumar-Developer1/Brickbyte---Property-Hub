...
apiClient.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('brickbyte_token');
    if (token) {
...
export const aiAPI = {
  search: (data: {
    ...
    const githubKey    = localStorage.getItem('brickbyte_github_key');
    const firecrawlKey = localStorage.getItem('brickbyte_firecrawl_key');
  ...
  validateKeys: (keys?: { githubKey?: string; firecrawlKey?: string }) => {
    const githubKey = (keys?.githubKey ?? localStorage.getItem('brickbyte_github_key') ?? '').trim();
    const firecrawlKey = (keys?.firecrawlKey ?? localStorage.getItem('brickbyte_firecrawl_key') ?? '').trim();
...
export const apiKeyStorage = {
  getGithubKey:    ()    => localStorage.getItem('brickbyte_github_key') || '',
  getFirecrawlKey: ()    => localStorage.getItem('brickbyte_firecrawl_key') || '',
  setGithubKey:    (key: string) => localStorage.setItem('brickbyte_github_key', key),
  setFirecrawlKey: (key: string) => localStorage.setItem('brickbyte_firecrawl_key', key),
  hasKeys: () => !!(localStorage.getItem('brickbyte_github_key') && localStorage.getItem('brickbyte_firecrawl_key')),
  clear: () => {
    localStorage.removeItem('brickbyte_github_key');
    localStorage.removeItem('brickbyte_firecrawl_key');
  },
};
