...
const SignUpPage: React.FC = () => {
  ...
  const handleSignUp = async (formData: any) => {
    ...
    try {
      ...
      const data = await register(fullName, formData.email, '', formData.password);
        ...
        <div className="bg-white border border-[#E6E0DA] rounded-2xl p-8 shadow-xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="font-manrope font-extralight text-sm text-[#4B5563]">
              Create an account to test the project workflow
            </p>
          </div>
          ...
          {successMessage && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
              <p className="font-manrope text-sm text-green-700">{successMessage}</p>
            </div>
          )}
